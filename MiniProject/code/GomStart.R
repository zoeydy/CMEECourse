...
GuessStart <- function(t, r_maxS, N_0S, KS, t_lagS, data){
  ...
  if (!is.na(out)){
    ...
    #print(AIC)
    return(list(AIC, rsq, r_maxS, N_0S, KS, t_lagS, predi_gom))
  }
  #print(out)
  return(out)
}
...
for (i in 1:length(unique(Data$ID))){
  ...
  for (j in 1:SampTime){
    ...
    start_id_list[[j]] <- result[1:6]
    plot_id_list[[j]] <- result[7]
  }
  ...
  colnames(start_id_df) <- c("AIC","RSq","r_max","N_0","K","t_lag","index")
  ...
  if (nrow(start_id_df[index,]) == 0) {
    start_id_df <- data.frame(ID=id,Model="gompertz",AIC=NA,RSq=NA,r_max=NA,N_0=NA,K=NA,t_lag=NA,index=NA)
    ...
    start_id_df <- data.frame(ID=id,model="gompertz",start_id_df[index,])
    colnames(start_id_df) <- c("ID","Model","AIC","RSq","r_max","N_0","K","t_lag","index")
    start_value_df <- rbind(start_value_df, start_id_df)
